...
export default function MarketsScreen() {
  ...
  const [sectorCurrency, setSectorCurrency] = useState('USD');
  const [selectedSector, setSelectedSector] = useState<SectorRow | null>(null);
  const [sectorSheetOpen, setSectorSheetOpen] = useState(false);
  const [heatmapColorMode, setHeatmapColorMode] = useState<HeatmapColorMode>('change');
  ...
  const loadTab = useCallback(async (t: Tab, m: Market, force = false) => {
    try {
      setError(null);
      if (t === 'overview') {
        // Paint indices + movers ASAP; sector rotation needs the full universe and is slower.
        ...
        void api.discoverSectorRotation(m, force).then((sec) => {
          setSectors(sec.sectors || []);
          setSectorCurrency(sec.currency || (m === 'IN' ? 'INR' : 'USD'));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cached.indices?.length || cached.gainers?.length) {
  ...
  const openSector = useCallback((name: string) => {
    const row = sectors.find((s) => s.sector === name);
    if (row) {
...
function OverviewTab({ indices, gainers, losers, sectors, breadthPct, onStock, onSeeMore, onSectors, onSectorSelect }: {
  ...
  return (
    <>
      ...
      {/* Breadth bar — hide until sector data arrives so overview isn't blocked */}
      {sectors.length > 0 ? (
      ...
      {topSectors.length > 0 ? (
        <>
          ...
          <SectorHeatmap sectors={topSectors} onSelect={onSectorSelect} />
        </>
      ) : null}
...
// ---------------- Sectors ----------------
function SectorsTab({ sectors, colorMode, onColorModeChange, onSectorSelect }: {
  sectors: SectorRow[];
  colorMode: HeatmapColorMode;
  onColorModeChange: (m: HeatmapColorMode) => void;
    ...
    <View style={{ paddingTop: 8 }}>
      <View style={styles.rotationRow}>
        <View style={styles.rotationCol}>
          ...
          {leaders.map((s) => (
            <Text key={s.sector} style={styles.rotationItem} numberOfLines={1}>
              {s.sector}{' '}
              <Text style={{ color: theme.colors.success }}>{s.avg_change_pct.toFixed(2)}%</Text>
              {s.avg_volume_growth_pct != null ? (
                <Text style={{ color: theme.colors.textSubtle, fontSize: 10 }}>
                  {' '}· Vol {s.avg_volume_growth_pct > 0 ? '+' : ''}{s.avg_volume_growth_pct.toFixed(0)}%
                </Text>
              ) : null}
            </Text>
          ))}
        ...
        <View style={styles.rotationCol}>
          ...
          {laggards.map((s) => (
            <Text key={s.sector} style={styles.rotationItem} numberOfLines={1}>
              {s.sector}{' '}
              <Text style={{ color: theme.colors.error }}>{s.avg_change_pct.toFixed(2)}%</Text>
              {s.avg_volume_growth_pct != null ? (
                <Text style={{ color: theme.colors.textSubtle, fontSize: 10 }}>
                  {' '}· Vol {s.avg_volume_growth_pct > 0 ? '+' : ''}{s.avg_volume_growth_pct.toFixed(0)}%
                </Text>
