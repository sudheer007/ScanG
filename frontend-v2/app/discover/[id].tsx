  ...
  return (
    <SortableDataTable
      ...
      columns={[
        ...
        { key: 'avg_volume_growth_pct', label: 'Vol %', width: 72, align: 'right', mono: true, sortValue: (r) => r.avg_volume_growth_pct ?? -999,
          render: (r) => <Text style={[cellText('right'), { color: changeColor(r.avg_volume_growth_pct), fontWeight: '700' }]}>
            {r.avg_volume_growth_pct != null ? `${r.avg_volume_growth_pct > 0 ? '+' : ''}${r.avg_volume_growth_pct.toFixed(1)}%` : '—'}
          </Text> },
        ...
        { key: 'volume_breadth_pct', label: 'Vol Br', width: 72, align: 'right', mono: true, sortValue: (r) => r.volume_breadth_pct ?? -999,
          render: (r) => <Text style={cellText('right')}>{r.volume_breadth_pct != null ? `${r.volume_breadth_pct.toFixed(0)}%` : '—'}</Text> },
