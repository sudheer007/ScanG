...
from ingestion.data_access import (
    batch_load_enrichment,
    build_events_from_mongo,
    load_symbol_enrichment,
...
def _yh_quote_batch(symbols: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    if not symbols:
        return {}
    # Cache full-universe batches (US / IN) so overview + movers share one fetch.
    cache_key = None
    if len(symbols) >= 50:
        cache_key = f"quotes:{len(symbols)}:{symbols[0]}:{symbols[-1]}"
        cached = QUOTE_BATCH_CACHE.get(cache_key)
    ...
    if cache_key is not None:
        QUOTE_BATCH_CACHE[cache_key] = out
    return out
...
async def get_bundle(symbol: str) -> Dict[str, Any]:
    ...
    if b["price"] is None and not chart_l.get("available"):
        return {"symbol": symbol, "error": "no_data"}
    enrichment = await asyncio.to_thread(load_symbol_enrichment, symbol)
    b = merge_enrichment_into_bundle(b, enrichment)
...
async def _build_market_universe(market: str) -> List[Dict[str, Any]]:
    ...
    mongo_batch = await asyncio.to_thread(batch_load_enrichment, symbols)
    ...
    for sym in symbols:
        ...
        if b["price"] is None:
            continue
        enrichment = mongo_batch.get(sym.upper()) or mongo_batch.get(sym) or {"available": False, "sections": {}}
        b = merge_enrichment_into_bundle(b, enrichment)
        BUNDLE_CACHE[sym] = b
        bundles.append(b)
...
def _fetch_events(symbol: str) -> Dict[str, Any]:
    ...
    enrichment = load_symbol_enrichment(symbol)
    mongo_events = build_events_from_mongo(enrichment)
    ...
    out = merge_events(out, mongo_events)
...
async def get_movers(market: str, mover_type: str = "gainers", limit: int = 15):
    # Prefer the full universe when already warm; otherwise use the lite path
    # so Markets overview does not wait on chart+summary for every ticker.
    key = f"universe_full:{market.upper()}"
    if key in UNIVERSE_BUNDLE_CACHE:
        universe = UNIVERSE_BUNDLE_CACHE[key]
    ...
    return await get_movers_lite(market, mover_type, limit)


def _quote_to_lite_stock(sym: str, q: Dict[str, Any], sparkline: Optional[List[float]] = None) -> Dict[str, Any]:
    return {
        "symbol": sym,
        "name": q.get("longName") or q.get("shortName") or sym,
        "sector": q.get("sector"),
        ...
        "volume": _safe(q.get("regularMarketVolume")),
        "avg_volume": _safe(q.get("averageDailyVolume3Month")),
        "pe": _safe(q.get("trailingPE")),
        "forward_pe": _safe(q.get("forwardPE")),
        "high_52w": _safe(q.get("fiftyTwoWeekHigh")),
        "low_52w": _safe(q.get("fiftyTwoWeekLow")),
        "sparkline": sparkline or [],
        "currency": q.get("currency") or ("INR" if sym.endswith(".NS") else "USD"),
        "exchange": q.get("fullExchangeName") or q.get("exchange"),
    }


async def get_movers_lite(market: str, mover_type: str = "gainers", limit: int = 15) -> List[Dict[str, Any]]:
    """Fast movers from quote batch + sparklines only for the top N (not full universe)."""
    symbols = get_universe(market)
    quote_map = await asyncio.to_thread(_yh_quote_batch, symbols)
    ranked: List[tuple] = []
    for sym in symbols:
    ...
    out: List[Dict[str, Any]] = []
    for (sym, q, _), cl in zip(top, chart_results):
        spark = (cl or {}).get("sparkline") or []
        out.append(_quote_to_lite_stock(sym, q, spark))
    return out
...
async def get_markets_overview_fast(market: str, limit: int = 10) -> Dict[str, Any]:
    ...
    quote_map, idx_quotes = await asyncio.gather(
        asyncio.to_thread(_yh_quote_batch, symbols),
        get_quotes(idx_syms),
    )
    for q in idx_quotes:
        q["name"] = idx_map.get(q["symbol"], q["symbol"])
    ...
    gainers_src = sorted(ranked, key=lambda x: x[2], reverse=True)[:limit]
    losers_src = sorted(ranked, key=lambda x: x[2])[:limit]
    spark_syms = list(dict.fromkeys([s for s, _, _ in gainers_src] + [s for s, _, _ in losers_src]))
    chart_results = await _gather_with_concurrency(spark_syms, _fetch_chart_layer, concurrency=20)
    spark_map = {sym: (cl or {}).get("sparkline") or [] for sym, cl in zip(spark_syms, chart_results)}
